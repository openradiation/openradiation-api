...
function openradiation_getItems()
{
    ...
    $.ajax({
    type: 'GET',
    url: 'https://open_geiger-c9-chsimon.c9.io/',
    //dataType: "jsonp",
    //jsonpCallback: "_test",
    cache: false,
    timeout: 5000,
...
function openradiation_refreshData() 
{
    ...
    for(var i in openradiation_items)
    {
        ...
        var htmlPopup = "<div>";
        htmlPopup += "hits number : <strong>" + openradiation_items[i].nb_coups + "</strong><br>";
        htmlPopup += timeValue2Str(openradiation_items[i].timestamp) + "<br><br>";
                   
        if (openradiation_items[i].login != null)
            htmlPopup += "by <strong>" + openradiation_items[i].login + "</strong><br>";
        if (openradiation_items[i].img != null)
            htmlPopup = htmlPopup + "<div><img class=\"img-rounded\" src=\"" + openradiation_items[i].img + "\"/></div><br>";       
        if (openradiation_items[i].altitude != null)
